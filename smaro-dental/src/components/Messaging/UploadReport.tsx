import React, {Fragment, useEffect, useRef} from "react";
import * as Yup from "yup";
import {useFormik, FormikValues} from "formik";
import {Dialog, Transition} from "@headlessui/react";
import {XMarkIcon as CloseIcon} from "@heroicons/react/16/solid";

import {api} from "@/api/api";
import useAuth from "@/hooks/useAuth";
import {formatDate} from "@/utils/utils";
import {ConversationTypes} from "@/types";
import {showErrorToast} from "@/utils/notify";

import LabelSelectBox from "../Common/LabelSelectBox";
import CustomFileUpload from "../Common/CustomFileUpload";

interface OwnProps {
    show: boolean;
    setShow: (value: boolean) => void;
    onSubmit: (params: any) => any;
    setIsLoading? : (value: boolean) => void;
    conversation: ConversationTypes
}
type Props = OwnProps;

const validationSchema = Yup.object().shape({
    media_url: Yup.string().required().label("File Url"),
    media_type: Yup.string().required().label("File Type"),
});

  interface SendMessageTypes {
    message: string;
    media_type?:string;
    media_url?:string;
    report_id:number;
  }


const UploadReport: React.FC<Props> = ({show,conversation, setShow, onSubmit}) => {

     const {user} = useAuth();
     const [reports,setReports] = React.useState<any[]>([]);
    const {values,touched, errors, setFieldValue, handleSubmit} = useFormik({
        validationSchema: validationSchema,
        initialValues: {
            report_id:0,
            media_url:0,
            media_type: 0,
        },
        onSubmit: (params: SendMessageTypes | FormikValues) => {
            onSubmit({
                conversation,
                ...params,
                message:"Reference Document uploaded",
            });
        }
    });

    const cancelButtonRef = useRef(null);

    const fetchReports = async (id:number)=>{
        try {
            const {data:apiData,status:apiStatus} = await api.get(api.endpoints.report_analysis.get.radiologist+"/"+id,{});
            if(apiStatus===200)
            {
                const {statusCode, data} = apiData;
                if(statusCode===200)
                {
                    setReports(data.map((item: { id:number; inserted_time: Date; short_code: string; report_analysis_id: number; })=>{
                          const time = formatDate(item.inserted_time);
                          const label =  item.short_code + " Report Date "+ time;

                        return {
                            value: item.id,
                            label: label
                        }
                    }));
                }
            }
        }catch (e){
            showErrorToast("You don't have any report assigned to you yet")
        }
    }

    const onSelect = (option: { value: number; })=>{
        void setFieldValue("report_id",option.value)
    }

    useEffect(() => {
       if(user?.id)
       {
            void (async ()=>{
                await fetchReports(user.id);
            })()
       }
    }, [user]);

    return (
        <Transition.Root show={show} as={Fragment}>
            <Dialog as="div" className="relative z-10 max-w-2xl" initialFocus={cancelButtonRef}
                    onClose={() => setShow(false)}>
                <Transition.Child
                    as={Fragment}
                    enter="ease-out duration-300"
                    enterFrom="opacity-0"
                    enterTo="opacity-100"
                    leave="ease-in duration-200"
                    leaveFrom="opacity-100"
                    leaveTo="opacity-0"
                >
                    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"/>
                </Transition.Child>

                <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <Transition.Child
                            as={Fragment}
                            enter="ease-out duration-300"
                            enterFrom="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                            enterTo="opacity-100 translate-y-0 sm:scale-100"
                            leave="ease-in duration-200"
                            leaveFrom="opacity-100 translate-y-0 sm:scale-100"
                            leaveTo="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                        >
                            <Dialog.Panel
                                style={{width: "650px"}}
                                className="relative transform rounded-lg bg-white text-left shadow-xl  transition-all sm:my-8 w-[calc(100%-62px)]">
                                <div className="bg-white px-4 p-1 sm:p-4 sm:pb-2 border-b-2">
                                    <div className="sm:flex sm:items-start">
                                        <div className="text-center sm:mt-0 sm:text-left w-full">
                                            <div className="flex justify-between w-full">
                                                <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900">
                                                    Upload Reference File
                                                </Dialog.Title>
                                                <CloseIcon className="h-6 w-6 text-gray-500" onClick={() => setShow(false)}/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="px-4 p-1">
                                     <LabelSelectBox
                                        width={250}
                                        label={"Select Report"}
                                        value={values.report_id}
                                        setValue={onSelect}
                                        options={reports}
                                        errorText={errors?.report_id}
                                        touched={touched.report_id}
                                     />
                                     <CustomFileUpload
                                         label="Upload Reference File"
                                         value={values.media_url} setValue={val=>setFieldValue("media_url",val)}
                                         touched={touched.media_url}
                                     />

                                </div>
                                <div className="h-[71px] border-t border-[#CED4DA] w-full flex gap-x-2 justify-end items-center px-4">
                                    <button onClick={() => setShow(false)} className="bg-[#6C757D] rounded-[4px] text-base font-semibold px-4 py-[10px]">
                                        Back
                                    </button>
                                    <button type="button" onClick={() => handleSubmit()} className="bg-[#0D6EFD] rounded-[4px] text-base font-semibold px-4 py-[10px]">
                                        Update
                                    </button>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
                    </div>
                </div>
            </Dialog>
        </Transition.Root>
    )
}

export default UploadReport;
